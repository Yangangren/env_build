...
class VehicleDynamics(object):
    ...
    def f_xu(self, states, actions, tau):  # states and actions are tensors, [[], [], ...]
        v_x, v_y, r, x, y, phi, front_wheel, a_x = states[:, 0], states[:, 1], states[:, 2], states[:, 3], states[:, 4], states[:, 5], states[:, 6], states[:, 7]
        phi = phi * np.pi / 180.
...
class EnvironmentModel(object):  # all tensors
    ...
    def rollout_out(self, actions):  # ref_points [#batch, 4]
        with tf.name_scope('model_step') as scope:
            ...
            veh2speed4real, reward_dict = self.compute_rewards(self.obses, self.actions)
    ...
    def compute_rewards(self, obses, actions):
        ...
        with tf.name_scope('compute_reward') as scope:
            ...
            for direc in range(0, self.road_info_dim, 2):
                shifted_x = obses_road[:, direc] - obses_ego[:, 3]
                shifted_y = obses_road[:, direc + 1] - obses_ego[:, 4]
                coordi_rotate_d_in_rad = obses_ego[:, 5] * np.pi / 180
                delta_x = shifted_x * tf.cos(coordi_rotate_d_in_rad) + shifted_y * tf.sin(coordi_rotate_d_in_rad)
                delta_y = -shifted_x * tf.sin(coordi_rotate_d_in_rad) + shifted_y * tf.cos(coordi_rotate_d_in_rad)

                veh2road4training += 1.0 * tf.where(logical_and(direc == 0, delta_y < 1.0), tf.square(delta_y - 1.0), tf.zeros_like(veh_infos[:, 0]))
                veh2road4real += 1.0 * tf.where(logical_and(direc == 0, delta_y < 1.0), tf.square(delta_y - 1.0), tf.zeros_like(veh_infos[:, 0]))
                veh2road4training += 1.0 * tf.where(logical_and(direc == 2, delta_y > -1.0), tf.square(delta_y - (-1.0)), tf.zeros_like(veh_infos[:, 0]))
                veh2road4real += 1.0 * tf.where(logical_and(direc == 2, delta_y > -1.0), tf.square(delta_y - (-1.0)), tf.zeros_like(veh_infos[:, 0]))


            rewards = 0.01 * devi_v + 0.8 * devi_lon + 0.8 * devi_lat + 30 * devi_phi + 0.02 * punish_yaw_rate + \
                      5 * punish_wheel + 0.4 * punish_delta_wheel + \
                      0.05 * punish_a_x + 0.1 * punish_delta_a_x

            punish_term_for_training = veh2veh4training + veh2road4training + veh2speed4training
            real_punish_term = veh2veh4real + veh2road4real + veh2speed4real
            ...
            reward_dict = dict(punish_wheel=punish_wheel,
                               ...
                               scaled_punish_delta_a_x=0.1 * punish_delta_a_x,
            ...
            return rewards, punish_term_for_training, real_punish_term, veh2veh4real, veh2road4real, veh2speed4real, reward_dict
    ...
    def compute_next_obses(self, obses, actions):
        ...
        next_obses_road = tf.gather(self.future_n_edges, indices=indexes, axis=-1, batch_dims=1)
    ...
    def _find_closest_point_batch(self, xs, ys, phis, paths):
        ego_lws = Para.L / 2.
        xs_front, ys_front = xs + ego_lws * np.cos(phis * np.pi / 180.), ys + ego_lws * np.sin(phis * np.pi / 180)
