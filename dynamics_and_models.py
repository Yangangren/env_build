...
from endtoend_env_utils import rotate_coordination, L, W, CROSSROAD_SIZE, LANE_WIDTH, LANE_NUMBER, \
    VEHICLE_MODE_LIST, EXPECTED_V, LIGHT
...
class EnvironmentModel(object):  # all tensors
    ...
    def reset(self, obses_ego, obses_other, veh_num, ref_indexes=None):  # input are all tensors
    ...
    def compute_next_obses(self, obses_ego, actions, obses_other):
        ...
        if self.mode != 'training':
            ...
            for light_idx, light_name in LIGHT.items():  # choose light
                for ref_idx, path in enumerate(self.ref_path.path_list[light_name]):  # choose path
                    filter = logical_and(light_infos == light_idx, ref_indexes == ref_idx)
        ...
        next_obses_ego = tf.concat([next_ego_infos, next_tracking_infos, light_infos], 1)
