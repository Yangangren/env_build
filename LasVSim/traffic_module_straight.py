...
class Traffic(object):
    ...
    def __init__(self, step_length, settings, map_type=None, traffic_type=None,
        ...
        if self.lasvsim_version == 'package':
            ...
            self.__path = module_path.replace('\\', '/') + "/Map/" + map_type + "/"
        else:
    ...
    def init(self, ego_position):
        ...
        while time_index < 500:
            traci.simulationStep()
            time_index += 1
        self.random_traffic = traci.junction.getContextSubscriptionResults('10')
        ...
        for veh in self.random_traffic:
            self.random_traffic[veh][87] = traci.vehicle.getRoute(vehID=veh)
        ...
        traci.vehicle.moveToXY(vehID='ego', edgeID='-gneE18',lane=traci.vehicle.getLaneIndex('ego'), x=self.__own_x +
                               self.ego_length / 2 *
    ...
    def get_vehicles(self):
        ...
        if self.__map_type == MAPS[0]:
            ...
            self.__own_lane_speed_limit = [34, 34,34, 34][
                self.veh_info_dict['ego'][traci.constants.VAR_LANE_INDEX]]
            self.__own_lane_pos = 9999.9
        else:
            raise Exception(self.__map_type + ' 该地图还未开放')
    ...
    def set_own_car(self, x, y, v, a):
        ...
        traci.vehicle.moveToXY(vehID='ego',edgeID='-gneE18', lane=-6, x=self.__own_x +
                               self.ego_length / 2 *
                               math.cos(math.radians(self.__own_a)),
                               y=self.__own_y + self.ego_length / 2 *math.sin(math.radians(self.__own_a)),
    ...
    def __initiate_traffic(self):
        ...
        for veh in self.random_traffic:
            ...
            #
            # traci.vehicle.setSpeed(vehID=veh,
