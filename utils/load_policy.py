...
class LoadPolicy(object):
    def __init__(self, exp_dir, iter):
        ...
        env = CrossroadEnd2endPiFix(training_task=self.args.env_kwargs_training_task,
        ...
        init_obs_ego = init_obs[:, :env.ego_info_dim + env.per_tracking_info_dim * (env.num_future_data+1)]
        init_obs_other = np.reshape(init_obs[:, env.ego_info_dim + env.per_tracking_info_dim * (env.num_future_data+1):], [-1, env.per_veh_info_dim])
